...
export default async function AppointmentDetailPage({ params }: PageProps) {
  ...
  const participantName = appointment.parishioner
    ? `${appointment.parishioner.firstName} ${appointment.parishioner.lastName}`
    : (appointment.publicRequesterName ?? "Public requester");
  const requestOrigin = appointment.requestedBy
    ? `${appointment.requestedBy.firstName} ${appointment.requestedBy.lastName}`
    : "Submitted from public parish page";
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                {!appointment.parishioner &&
                  appointment.publicRequesterEmail && (
                    <p className="text-sm text-muted-foreground mt-1">
                      {appointment.publicRequesterEmail}
                    </p>
                  )}
                {!appointment.parishioner &&
                  appointment.publicRequesterPhone && (
                    <p className="text-sm text-muted-foreground">
                      {appointment.publicRequesterPhone}
                    </p>
                  )}
              ...
              <div>
                ...
                <p className="text-base font-medium">{requestOrigin}</p>
                ...
                {!appointment.requestedBy &&
                  appointment.source === "PUBLIC" && (
                    <p className="text-sm text-muted-foreground mt-1">
                      This request was submitted by a public visitor.
                    </p>
                  )}
              </div>
              {appointment.availability && (
                <div>
                  ...
                  <p className="text-base font-medium">
                    {appointment.availability.title}
                  </p>
                  <p className="text-sm text-muted-foreground mt-1">
                    {formatDateTime(appointment.availability.startTime)} at{" "}
