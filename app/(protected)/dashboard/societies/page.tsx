...
export default async function SocietiesPage() {
  ...
  let pendingRequestCountMap = new Map<string, number>();
  ...
  if (canReviewRequests) {
    const pendingRequests = await db.societyJoinRequest.findMany({
      where: {
        status: "PENDING",
    ...
    for (const req of pendingRequests) {
      pendingRequestCountMap.set(
        req.societyId,
        (pendingRequestCountMap.get(req.societyId) ?? 0) + 1,
      );
    }
  }
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {societies.length === 0 ? (
          ...
          societies.map((society: SocietyWithRelations) => {
            const joinStatus = isParishioner
              ? membershipSet.has(society.id)
                ? "MEMBER"
                : (joinRequestMap.get(society.id) ?? "NONE")
              : null;
            const pendingRequests = pendingRequestCountMap.get(society.id) ?? 0;
              ...
              <Card
                ...
                <CardHeader>
                  <div className="flex justify-between items-start">
                    ...
                    {canReviewRequests && pendingRequests > 0 && (
