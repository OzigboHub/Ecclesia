...
export default async function OrganizationDetailPage({
	...
	const targetOrgId = isParishAdmin ? session.user.organizationId : id;

	const result = await getOrganizationDetailedView(targetOrgId, 5);

	if (!result.success || !result.data) {
		redirect("/dashboard/admin/organizations");
	}

	if (isParishAdmin && targetOrgId !== session.user.organizationId) {
		redirect("/dashboard");
	}
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex items-start justify-between">
				<div className="space-y-2">
					...
					>
						<Button variant="ghost" size="sm" className="mb-2">
							...
							Back to{" "}
							{isParishAdmin ? "Dashboard" : "Organizations"}
						</Button>
					</Link>
					...
					<div className="flex items-center gap-4 mt-2">
						...
						<Link
							href={`/dashboard/admin/organizations/${targetOrgId}/settings`}
						>
				...
				<div className="flex gap-2">
					{isSuperAdmin && (
						<>
							<Link
								href={`/dashboard/admin/organizations/${targetOrgId}/edit`}
							>
								<Button>
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<MetricCard
					title="Parishioners"
					count={counts.totalParishioners}
					icon={UserCircle}
				/>
				<MetricCard
					title="Societies"
					count={counts.totalSocieties}
					icon={UsersRound}
				/>
				{isParish ?
					<MetricCard
						title="Outstations"
						count={counts.totalOutstations}
						icon={Building2}
					/>
				:	<MetricCard
						title="Sacramental"
						count={
							counts.totalBaptisms +
							counts.totalConfirmations +
							counts.totalMarriages
						}
						icon={BookOpen}
					/>
				}
				...
				<div className="lg:col-span-2 space-y-6">
					...
					<Card>
						<CardHeader className="flex flex-row items-center justify-between">
							...
							{counts.totalUsers > 5 && (
								<Link
									href={`/dashboard/admin/organizations/${targetOrgId}/users`}
								>
									<Button variant="outline" size="sm">
										View All
									</Button>
					...
					<Card>
						<CardHeader className="flex flex-row items-center justify-between">
							...
							{counts.totalParishioners > 5 && (
								<Link
									href={`/dashboard/admin/organizations/${targetOrgId}/parishioners`}
								>
									<Button variant="outline" size="sm">
										View All
									</Button>
					...
					<Card>
						...
						<CardContent className="space-y-4">
							<div className="grid grid-cols-3 gap-2">
								<SacramentBrief
									count={counts.totalBaptisms}
									label="Baptisms"
								/>
								<SacramentBrief
									count={counts.totalConfirmations}
									label="Confirm."
								/>
								<SacramentBrief
									count={counts.totalMarriages}
									label="Marriages"
								/>
							</div>
							...
							<div className="space-y-3 mt-4">
								...
								{[
									...
									.map((record, k) => (
										<div
											key={record.id || k}
											className="text-xs flex justify-between p-2 rounded bg-muted/50"
										>
					...
					<Card>
						<CardHeader className="flex flex-row items-center justify-between pb-2">
							...
							{counts.totalSocieties > 5 && (
								<Link
									href={`/dashboard/admin/organizations/${targetOrgId}/societies`}
								>
									<Button
										variant="link"
										size="sm"
						...
						<CardContent>
							...
							:	<div className="space-y-1">
									{societies.map((s) => (
										<div
											key={s.id}
											className="text-xs p-2 rounded hover:bg-muted transition-colors"
										>
											{s.name}
										</div>
									))}
					...
					<CardHeader className="flex flex-row items-center justify-between">
						...
						<div className="flex gap-2">
							{counts.totalOutstations > 5 && (
								<Link
									href={`/dashboard/admin/organizations/${targetOrgId}/outstations`}
								>
									<Button variant="outline" size="sm">
										View All
									</Button>
							...
							<Link
								href={`/dashboard/admin/organizations/${targetOrgId}/new-outstation`}
							>
								<Button size="sm">
...
function SacramentBrief({ count, label }: { count: number; label: string }) {
	return (
		<div className="flex flex-col items-center justify-center p-2 rounded-lg bg-primary/5 border border-primary/10">
			...
			<span className="text-[9px] uppercase tracking-tighter font-semibold text-muted-foreground">
				{label}
			</span>
		</div>
	);
