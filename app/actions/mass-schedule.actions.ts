...
import { MassType, DayOfWeek, MassScheduleTemplate } from '@prisma/client';
...
export type MassScheduleTemplateInput = z.infer<typeof massScheduleTemplateSchema>;
    ...
    try {
        ...
        const template = await db.massScheduleTemplate.create({
            data: {
                ...
                effectiveFrom: validated.data.effectiveFrom ? new Date(validated.data.effectiveFrom) : null,
                effectiveUntil: validated.data.effectiveUntil ? new Date(validated.data.effectiveUntil) : null,
            },
        });
...
export async function updateMassScheduleTemplate(
    ...
    try {
        ...
        // validate partial
        const validated = massScheduleTemplateSchema.partial().parse(data);
        ...
        await db.massScheduleTemplate.update({
            ...
            data: {
                ...
                effectiveUntil: validated.effectiveUntil ? new Date(validated.effectiveUntil) : undefined,
            },
        });
...
export async function deleteMassScheduleTemplate(id: string): Promise<ActionResponse> {
    try {
        const session = await auth();
        if (!session || !['SUPER_ADMIN', 'PARISH_ADMIN'].includes(session.user.role)) {
            return { success: false, message: 'Unauthorized' };
        ...
        await db.massScheduleTemplate.delete({
            where: { id, organizationId: session.user.organizationId },
        });
