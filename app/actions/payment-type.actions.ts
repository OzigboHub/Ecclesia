...
export async function deletePaymentType(id: string): Promise<ActionResponse> {
	try {
		...
		if (!PAYMENT_TYPE_DELETE_ROLES.includes(session.user.role)) {
			return {
				success: false,
				message: "Only parish admins can delete payment types",
			};
		}
...
export async function getMassPaymentTypes(
	...
	try {
		...
		return {
			success: false,
			message: "Failed to retrieve mass payment types",
		};
	}
}
...
export async function setMassPaymentTypes(
	...
	try {
		...
		return {
			success: false,
			message: "Failed to update mass payment types",
		};
	}
}
...
export async function ensureDefaultPaymentTypes(
	...
	for (const defaults of DEFAULT_SUNDAY_PAYMENT_TYPES) {
		...
		if (!pt) {
			...
			const creatorId = adminUser?.id ?? fallbackUser?.id;
			if (!creatorId) {
				throw new Error(
					"No user found to attribute payment type creation",
				);
			}
