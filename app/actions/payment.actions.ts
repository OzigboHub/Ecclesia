	...
	try {
		...
		if (
			...
			return {
				success: false,
				message:
					"Access Denied: You can only view your own payment records",
			};
		}
...
export async function createPayment(
	...
	try {
		...
		const { purpose, paymentMethod, societyId, ...paymentData } =
			parsed.data;
		...
		if (purpose === "SOCIETY_DUES") {
			...
			if (
				...
				return {
					success: false,
					message:
						"Society dues payments must be made by bank transfer",
				};
			}
		...
		if (!featureCheck.enabled) {
			return {
				...
				message:
					...
					"This payment feature is not enabled",
			};
		}
		...
		if (!payerName && purpose === "SOCIETY_DUES" && finalParishionerId) {
			...
			if (parishioner) {
				payerName =
					`${parishioner.firstName ?? ""} ${parishioner.lastName ?? ""}`.trim();
			}
		}
		if (!payerName) {
			payerName = "Guest";
		...
		const payment = await db.payment.create({
			data: {
				...
				paymentDate:
					formPaymentDate ? new Date(formPaymentDate) : undefined,
				receiptNumber,
		...
		if (payment.paymentStatus === "COMPLETED") {
			await sendParishEventNotification({
				...
				body: `${payment.payerName} completed a ${payment.purpose.toLowerCase().replace(/_/g, " ")} payment of NGN ${payment.amount.toLocaleString()}.`,
				url: "/payments",
			});
		}
