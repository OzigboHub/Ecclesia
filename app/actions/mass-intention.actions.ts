...
async function resolveMassIntentionOwnerUserIds(params: {
	...
	if (params.contactEmail) {
		ownerEmails.add(params.contactEmail);
	}

	if (params.parishionerId) {
		const parishioner = await db.parishioner.findFirst({
			where: {
				id: params.parishionerId,
		...
		if (parishioner?.email) {
			ownerEmails.add(parishioner.email);
		}
	}
	...
	try {
		...
		try {
			await sendParishEventNotification({
				...
				title: "New mass intention submitted",
				body: `${result.requestedBy} submitted a ${result.intentionType.toLowerCase().replace(/_/g, " ")} mass intention for review.`,
				url: "/mass-intentions",
			});
		} catch (submitNotificationError) {
			console.error(
				"Failed to send mass intention submission notification:",
	...
	try {
		...
		if (!canManageMassIntentions(session.user.role)) {
			return {
				success: false,
				message:
					"You do not have permission to approve mass intentions",
			};
		}
		...
		try {
			...
			console.error(
				"Failed to write mass intention approval audit log:",
				auditError,
			);
		}
		...
		try {
			...
			console.error(
				"Failed to revalidate mass intention approval paths:",
				revalidateError,
			);
		}
		...
		try {
			ownerUserIds = await resolveMassIntentionOwnerUserIds({
				organizationId: session.user.organizationId,
				parishionerId: massIntention.parishionerId,
				contactEmail: massIntention.contactEmail,
			});
		} catch (ownerResolveError) {
			console.error(
				"Failed to resolve mass intention owner user IDs:",
				ownerResolveError,
			);
		}

		const approverRole = formatRoleLabel(session.user.role);
		const intentionTypeLabel = massIntention.intentionType
			.toLowerCase()
			.replace(/_/g, " ");
		...
		if (ownerUserIds.length > 0) {
			try {
				await sendParishEventNotification({
					organizationId: session.user.organizationId,
					organizationName: massIntention.organization.name,
					audience: "ALL_ORG_USERS",
					targetUserIds: ownerUserIds,
					title: "Your mass intention has been approved",
					body: `Your ${intentionTypeLabel} intention was approved by a ${approverRole}.`,
					url: `/mass-intentions/${id}`,
				});
			} catch (ownerNotificationError) {
				console.error(
					"Failed to send owner mass intention approval notification:",
		...
		try {
			await sendParishEventNotification({
				...
				excludeUserIds: ownerUserIds,
				title: "New mass intention in your parish",
				body: `A new ${intentionTypeLabel} mass intention has been sent into the parish.`,
				url: "/mass-intentions",
			});
		} catch (parishionersNotificationError) {
			console.error(
				"Failed to send parishioners mass intention approval notification:",
				parishionersNotificationError,
			);
		}
...
export async function exportMassIntentions(): Promise<ActionResponse<string>> {
	try {
		...
		const rows = massIntentions.map((mi) => [
			...
			mi.mass?.date ?
				new Date(mi.mass.date).toLocaleDateString("en-NG")
			:	"",
			...
			mi.parishioner ?
				escapeCSV(
					`${mi.parishioner.firstName} ${mi.parishioner.lastName}`,
				)
			:	"",
			escapeCSV(mi.notes),
			new Date(mi.createdAt).toLocaleDateString("en-NG"),
			mi.approvedAt ?
				new Date(mi.approvedAt).toLocaleDateString("en-NG")
			:	"",
		]);
