	...
	try {
		...
		if (existingAtSameTime) {
			return {
				success: false,
				message:
					"A mass already exists at this time for the selected day.",
			};
		}
...
export async function runMassGeneration(
	...
	try {
		...
		const start =
			startDate ?
				parseMassDayInput(startDate)
			:	toUtcDayStart(new Date());
		const end =
			endDate ? parseMassDayInput(endDate) : addUtcDays(start, 30);
...
export async function getMassDays(
	...
	try {
		...
		const dates = Array.from(
			new Set(masses.map((m) => m.date.toISOString().slice(0, 10))),
		);
