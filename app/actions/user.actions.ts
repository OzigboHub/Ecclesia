  ...
  try {
    ...
    if (role === "OUTSTATION_ADMIN") {
      ...
      if (!paystackParsed.success) {
        const fieldErrors = paystackParsed.error.flatten().fieldErrors || {};
      ...
      if (!paystackResult.success) {
        return {
          success: false,
          message:
            paystackResult.message || "Failed to create payment subaccount",
...
export async function updateUser(
  ...
  try {
    ...
    const updateData: Partial<typeof parsed.data> = { ...parsed.data };

    if (
      parsed.data.organizationId &&
      parsed.data.organizationId !== existingUser.organizationId
    ) {
      ...
      if (!targetOrganization) {
        return {
          ...
          errors: {
            organizationId: ["Invalid organization selection"],
          },
        };
      }

      if (session.user.role === "PARISH_ADMIN") {
        const managedOrganizationIds = await getManagedOrganizationIds(session);
        if (!managedOrganizationIds.includes(parsed.data.organizationId)) {
          return {
            success: false,
            message:
              "You do not have permission to move this user to the selected organization",
            errors: {
              organizationId: ["Invalid organization selection"],
            },
          };
    ...
    if (
      ...
      await db.auditLog.create({
        data: {
          ...
          performedBy: session.user.id,
          details: {
            fromOrganizationId: existingUser.organizationId,
            toOrganizationId: parsed.data.organizationId,
          },
        },
      });
    }
