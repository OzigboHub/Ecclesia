...
async function checkLiveStreamingEnabled(
...
export async function getLiveStream(id: string): Promise<ActionResponse> {
  try {
    const session = await auth();
    if (!session) return { success: false, message: "Unauthorized" };
...
export async function updateLiveStream(
  ...
  try {
    ...
    const stream = await db.liveStream.update({
      ...
      data: {
        ...rest,
        ...(massId !== undefined ? { massId: massId || null } : {}),
      },
    });
...
export async function deleteLiveStream(id: string): Promise<ActionResponse> {
  try {
    const session = await auth();
    if (!session) return { success: false, message: "Unauthorized" };
