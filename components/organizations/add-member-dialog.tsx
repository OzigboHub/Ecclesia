...
export function AddMemberDialog({ organizationId }: { organizationId: string }) {
    ...
    const [selectedParishioner, setSelectedParishioner] = useState<Parishioner | null>(null);
    ...
    const handleSubmit = () => {
        ...
        startTransition(async () => {
            try {
                await addMember(organizationId, selectedParishioner.id, selectedRole);
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Search for a parishioner and assign them a role in the organization.
                    </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                    <div className="flex flex-col gap-2">
                        <label className="text-sm font-medium">Parishioner</label>
                        <Popover open={openCombobox} onOpenChange={setOpenCombobox}>
                            ...
                            <PopoverContent className="w-[400px] p-0">
                                <Command shouldFilter={false}>
                                    ...
                                    <CommandList>
                                        <CommandEmpty>
                                            {isSearching ? 'Searching...' : 'No parishioner found.'}
                                        </CommandEmpty>
                                        <CommandGroup>
                                            {parishioners.map((parishioner) => (
                                                ...
                                                >
                                                    <Check
                                                        className={cn(
                                                            "mr-2 h-4 w-4",
                                                            selectedParishioner?.id === parishioner.id
                                                                ? "opacity-100"
                                                                : "opacity-0"
                                                        )}
                                                    />
                ...
                <DialogFooter>
                    ...
                    <Button onClick={handleSubmit} disabled={!selectedParishioner || isPending}>
