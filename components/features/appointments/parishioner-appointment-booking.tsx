...
import { bookAppointmentAsParishioner } from "@/app/actions/appointment.actions";
...
export function ParishionerAppointmentBooking({
  ...
  const onSubmit = (data: ParishionerAppointmentInput) => {
    startTransition(async () => {
      const result = await bookAppointmentAsParishioner(data);
      if (!result.success) {
        toast.error(result.message);
        if (result.errors) {
          Object.entries(result.errors).forEach(([field, messages]) => {
            form.setError(field as keyof ParishionerAppointmentInput, {
              type: "server",
          ...
          <CardContent>
            {availabilities.length === 0 ? (
              <p className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
                ...
                later or contact your parish office.
              </p>
            ) : (
              <div className="space-y-2">
                {availabilities.map((slot) => {
                  const isSelected = slot.id === selectedAvailabilityId;
                  return (
                    <button
                      type="button"
                      key={slot.id}
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {slot.remainingBookings} spaces remaining
                      </p>
                    </button>
                  );
                })}
          ...
          <CardContent>
            {selectedAvailability ? (
              <div className="space-y-4">
                <div>
                  <p className="font-semibold">{selectedAvailability.title}</p>
                  <p className="text-sm text-muted-foreground">
                    {selectedAvailability.type.replace(/_/g, " ")}
                  </p>
                </div>
                <div className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      ...
                      -{" "}
                      {format(new Date(selectedAvailability.endTime), "h:mm a")}
                    </span>
                  </div>
                </div>
                <p className="text-sm text-muted-foreground">
                ...
                <p className="text-sm text-muted-foreground">
                  ...
                  {selectedAvailability.maxBookings} spaces remaining.
                </p>
                <Button
                  type="button"
      ...
      <Modal
        ...
        <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="parish-title">Appointment title</Label>
              <Input
                id="parish-title"
                {...form.register("title")}
                disabled={isPending}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    <SelectContent>
                      {appointmentTypeEnum.options.map((type) => (
