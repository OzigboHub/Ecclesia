...
export function MassIntentionCalendar({
	...
	const handleOrganizationChange = (orgId: string) => {
		...
		startTransition(async () => {
			const result = await getMassesInRange(
				format(subMonths(startOfMonth(currentDate), 1), "yyyy-MM-dd"),
				format(addMonths(endOfMonth(currentDate), 1), "yyyy-MM-dd"),
				orgId,
			);
	...
	const intentionsByDate = useMemo(() => {
		...
		intentions.forEach((intention) => {
			if (
				...
				(!selectedOrganizationId ||
					intention.organizationId === selectedOrganizationId)
			) {
				const key = new Date(intention.mass.date)
					.toISOString()
					.split("T")[0];
		...
		return map;
	}, [intentions, selectedOrganizationId]);
	...
	const massesForSelectedDate =
		selectedDateKey ? massesByDate.get(selectedDateKey) || [] : [];
	const selectedMass =
		selectedMassId ?
	...
	const filteredIntentions = useMemo(() => {
		return intentions.filter((intention) => {
			...
			return isSameDay(new Date(intention.mass.date), selectedDate);
		});
	}, [intentions, selectedDate, selectedOrganizationId]);
						...
						<Card className="border-dashed">
							<CardContent className="flex flex-col items-center justify-center py-16 text-center">
								...
								<p className="text-xs text-muted-foreground mt-1">
									Click any future date on the calendar to
									view available mass times.
								</p>
							</CardContent>
						</Card>
					:	<Card>
							...
							<CardContent className="pt-4 space-y-3">
								{massesForSelectedDate.length === 0 ?
									<div className="text-center py-8">
										...
										<p className="text-xs text-muted-foreground mt-1">
											Try another date or contact the
											parish office.
										</p>
									</div>
						...
						<CardContent className="pt-4">
							{canReviewIntentions && (
								<p className="text-xs text-muted-foreground mb-3">
									Tap an intention to view details and approve
									or reject it.
								</p>
							)}
							<div className="space-y-3">
								{filteredIntentions.length === 0 && (
									<p className="text-sm text-muted-foreground">
										{selectedDate ?
											"No intentions found for the selected day."
										:	"No intentions found."}
									</p>
								)}
