...
export function AppointmentEditForm({ appointment }: AppointmentEditFormProps) {
	...
	const extractNotes = (description: string | null): string => {
		...
		const notesMatch = description.match(/Additional Notes:\s*(.+)$/s);
					...
					<div className='grid grid-cols-2 gap-4'>
						<div className='space-y-2'>
							...
							{staffForm.formState.errors.firstName && (
								<p className='text-sm text-destructive'>
									{staffForm.formState.errors.firstName.message}
								</p>
							)}
						</div>
						<div className='space-y-2'>
							...
							{staffForm.formState.errors.lastName && (
								<p className='text-sm text-destructive'>
									{staffForm.formState.errors.lastName.message}
								</p>
							)}
						</div>
					</div>
						...
						<Controller
							...
							render={({ field }) => (
								...
								>
									...
									<SelectContent>
										<SelectItem value='PARISH_STAFF'>Parish Staff</SelectItem>
