...
const FEATURE_BYPASS_ROLES = new Set(["SUPER_ADMIN", "PARISH_ADMIN"]);

export function canBypassFeatureToggle(role?: string | null): boolean {
  return !!role && FEATURE_BYPASS_ROLES.has(role);
}
...
/**
 * Check if a specific feature is enabled for a user role (with admin bypass support)
 */
export async function isFeatureEnabledForRole(
  organizationId: string,
  feature: FeatureName,
  role?: string | null,
): Promise<boolean> {
...
export async function canEnableFeature(
  ...
  const { featureDependencies, getDefaultFeatureValue } =
